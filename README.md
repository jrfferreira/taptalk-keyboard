...
| Selectable keyboard layouts — 10, see below | ✅ implemented; hardware validation pending |
| Cyrillic, Greek and other non-Latin layouts | ❌ next, same mechanism |

The setup page asks which layout the *computer* is set to — the host decodes
our keystrokes through its own layout, so the two must match. Ten layouts ship:
US, UK, US International, Portuguese (Brazil ABNT2 and Portugal), Spanish
(Spain and Latin America), French AZERTY, German QWERTZ, and Italian. Accents
type natively (dead keys where the layout composes them); a character the
layout cannot produce falls back to its bare letter — `ação` arrives as `acao`
on US — and emoji are skipped rather than mangled.

Tables are generated from Microsoft's layout definitions by
`tools/gen_keymap.py`, never written from memory, so adding a layout is one
generator run plus an `<option>` in the portal. Cyrillic and Greek work the
same way (each letter is a key position when the host uses that layout) and
are on the roadmap. Chinese, Japanese and Korean cannot be typed by any
driverless keyboard: those scripts require an IME on the host, which composes
text interactively on screen. AltGr symbols assume a Windows or Linux host;
macOS arranges its Option combinations differently.
...
```sh
cd test/host && make test        # 19,738 checks across 10 suites
```
...
components/core/   pure C, no esp_* headers, host-testable
  ...
  keymap*.c        codepoint -> HID keystroke sequence (dead keys need two);
                   most are generated by tools/gen_keymap.py
...
tools/             idf.sh (docker build), flash.sh (native esptool), release.sh,
                   gen_keymap.py (Windows .klc -> keymap table)
