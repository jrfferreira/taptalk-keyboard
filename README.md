...
| | |
|---|---|
| Pure logic (WAV, keymap, multipart, state machine) | done, unit + interop tested |
...
Needs Docker. There is no ESP-IDF on the host.

```sh
cp main/secrets.example.h main/secrets.h   # then fill in your Wi-Fi
tools/idf.sh build
```
