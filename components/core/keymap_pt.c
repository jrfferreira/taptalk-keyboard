 ...
 *
 * Transcribed from Microsoft's KBDPO tables (kbdpo.dll 10.0.25393.1, via
...
/* The five Portuguese dead keys, as (modifier, usage) pairs. */
#define DEAD_ACUTE 0, HID_KEY_RBRACKET     /* ´ */
#define DEAD_GRAVE S, HID_KEY_RBRACKET     /* ` */
#define DEAD_TILDE 0, HID_KEY_NONUS_HASH   /* ~ */
#define DEAD_CIRC  S, HID_KEY_NONUS_HASH   /* ^ */
#define DEAD_TREMA G, HID_KEY_LBRACKET     /* ¨ rides AltGr on the +* key */

static const keymap_entry_t pt_entries[] = {
    ONE('\t', 0, HID_KEY_TAB),
    ONE('\n', 0, HID_KEY_ENTER),
    ONE(' ', 0, HID_KEY_SPACE),
    ...
    ONE('%', S, 0x22), ONE('&', S, 0x23), ONE('/', S, 0x24), ONE('(', S, 0x25),
    ONE(')', S, HID_KEY_9), ONE('=', S, HID_KEY_0),

    ONE('\'', 0, HID_KEY_MINUS), ONE('?', S, HID_KEY_MINUS),
    ONE(0x00AB, 0, HID_KEY_EQUAL), ONE(0x00BB, S, HID_KEY_EQUAL), /* « » */
    ONE('+', 0, HID_KEY_LBRACKET), ONE('*', S, HID_KEY_LBRACKET),
    ONE(0x00E7, 0, HID_KEY_SEMICOLON), /* ç has its own key */
    ONE(0x00C7, S, HID_KEY_SEMICOLON), /* Ç */
    ONE(0x00BA, 0, HID_KEY_APOSTROPHE), /* º */
    ONE(0x00AA, S, HID_KEY_APOSTROPHE), /* ª */
    ONE('\\', 0, HID_KEY_GRAVE), ONE('|', S, HID_KEY_GRAVE),
    ONE(',', 0, HID_KEY_COMMA), ONE(';', S, HID_KEY_COMMA),
    ONE('.', 0, HID_KEY_PERIOD), ONE(':', S, HID_KEY_PERIOD),
    ONE('-', 0, HID_KEY_SLASH), ONE('_', S, HID_KEY_SLASH),
    ...
    ONE(0x00A3, G, 0x20), /* £ */
    ONE(0x00A7, G, 0x21), /* § */
    ONE(0x20AC, G, 0x08), /* € on AltGr+E (Windows also offers AltGr+5) */
    ONE('{', G, 0x24), ONE('[', G, 0x25), ONE(']', G, 0x26), ONE('}', G, HID_KEY_0),
    ...
    TWO(0x00FF, DEAD_TREMA, 0, 0x1C),              /* ÿ — KBDPO has no Ÿ */

    TWO(0x00B4, DEAD_ACUTE, 0, HID_KEY_SPACE), /* standalone ´ */
    TWO(0x00A8, DEAD_TREMA, 0, HID_KEY_SPACE), /* standalone ¨ */
};

static bool pt_lookup(uint32_t cp, hid_seq_t *out)
{
    return keymap_table_lookup(pt_entries, sizeof(pt_entries) / sizeof(pt_entries[0]), cp, out);
}

const keymap_layout_t keymap_pt = {
    .name   = "pt",
    .lookup = pt_lookup,
};
